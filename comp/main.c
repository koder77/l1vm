...
S2 parse_line (U1 *line, S2 start, S2 end)
{
	...
	*/
	...
	// first code line, load zero into register 0
	if (code_line == 0)
	{
		init_registers ();
					...
					else
					{
						...
						if (last_arg >= 0)
						{
							...
							if (ok == 0)
							{
								...
								if (strcmp ((const char *) ast[level].expr[j][last_arg], "intr0") == 0 || strcmp ((const char *) ast[level].expr[j][last_arg], "intr1") == 0)
								{
									printf ("intr0/intr1 expression...\n ");
