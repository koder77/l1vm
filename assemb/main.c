...
S2 write_data_byte (S8 offset ALIGN, S8 val ALIGN)
{
	if (offset < data_max)
	{
        data[offset] = (U1) val;
...
S2 write_data_word (S8 offset, S2 val)
{
    U1 *bptr = (U1 *) &val;
    
    if (offset + sizeof (S2) < data_max)
    {
...
S2 write_data_doubleword (S8 offset ALIGN, S4 val)
{
    U1 *bptr = (U1 *) &val;
     
    if (offset + sizeof (S4) < data_max)
    {
#if MACHINE_BIG_ENDIAN
        data[offset] = *bptr;
        bptr++;
...
S2 write_data_quadword (S8 offset ALIGN, S8 val ALIGN)
{
    U1 *bptr = (U1 *) &val;
     
    if (offset + sizeof (S8) < data_max)
    {
#if MACHINE_BIG_ENDIAN
        data[offset] = *bptr;
        bptr++;
...
S2 write_data_doublefloat (S8 offset ALIGN, F8 val ALIGN)
{
    U1 *bptr = (U1 *) &val;
     
    if (offset + sizeof (F8) < data_max)
    {
#if MACHINE_BIG_ENDIAN
        data[offset] = *bptr;
        bptr++;
            ...
            if (args[0][0] == '@')
            {
                ...
                if (checkdigit (args[2]) == 1)
                {
                    if (t_var.digitstr_type == DOUBLEFLOAT)
                    {
                        ...
                        write_data_doublefloat (offset, datad);
                    ...
                    else
                    {
                        ...
                        {
                            case BYTE:
                                if (write_data_byte (offset, (U1) datai) != 0)
								{
                            ...
                            case WORD:
                                if (write_data_word (offset, (S2) datai) != 0)
								{
									printf ("error: line %lli: out of data memory!\n", linenum);
								}
                                break;
                            ...
                            case DOUBLEWORD:
                                if (write_data_doubleword (offset, (S4) datai) != 0)
								{
									printf ("error: line %lli: out of data memory!\n", linenum);
								}
                                break;
                            ...
                            case QUADWORD:
                                if (write_data_quadword (offset, datai) != 0)
								{
									printf ("error: line %lli: out of data memory!\n", linenum);
								}
                                break;
                    ...
                    data_ind += data_info[data_info_ind].type_size;
                }
                else
                {
                    printf ("line: %lli DATA STRING\n", linenum);
                    if (write_data_string (offset, args[2]) != 0)
					{
						printf ("error: line %lli: out of data memory!\n", linenum);
					}
					data_info[data_info_ind].offset = offset;
					data_ind += data_info[data_info_ind].size;
                }
            }
            else
            {
                        ...
                        for (j = 1; j <= arg_ind; j++)
                        {
                            switch (opcode[i].type[j - 1])
                            {
                                ...
                                case D_REG:
                                    ...
                                    {
                                        printf ("error: line %lli: argument %i not a number!\n", linenum, j);
                                        return (1);
                                    }
                                    break;
...
S2 dump_object (U1 *name)
{
	...
	data_size = data_ind;
	data_ind = 0;
	write_data_quadword (0, data_size);
