#!/bin/bash
echo "L1VM - build clang 10.0.0 toolchain 1.2"
...
wget https://github.com/llvm/llvm-project/releases/download/llvmorg-10.0.0/llvm-10.0.0.src.tar.xz.sig
wget https://github.com/llvm/llvm-project/releases/download/llvmorg-10.0.0/clang-10.0.0.src.tar.xz.sig
echo "Downloading llvm.org gpg key."
wget https://releases.llvm.org/10.0.0/hans-gpg-key.asc
...
gpg --verify llvm-10.0.0.src.tar.xz.sig llvm-10.0.0.src.tar.xz
if [ $? -gt 0 ]
then
...
cd build
echo "configure..."
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=~/l1vm-clang-10.0.0 ../
echo "make..."
make -j4
echo "install..."
make install
