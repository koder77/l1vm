...
U1 draw_text_ttf (SDL_Surface *surface, S2 screennum, U1 *textstr, Sint16 x, Sint16 y, Uint8 r, Uint8 g, Uint8 b)
{
	...
	textsurf = TTF_RenderText_Blended (font, (const char *) textstr, 0, color);
	if (textsurf == NULL)
	{
	...
	SDL_QueryTexture(texture, NULL, NULL, &dstRectTex.w, &dstRectTex.h);

	dstRect.x = (float)dstRectTex.x;
	dstRect.y = (float)dstRectTex.y;

	SDL_RenderTextureRotated(renderer, texture, NULL, &dstRect, 0., NULL, flip);
	*/
	return (TRUE);
}
...
int do_copy_surface (SDL_Renderer *dest_renderer, SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
	...
	for (src_y = srcrect_y; src_y < srcrect_y + srcrect_h; src_y++)
	{
		...
		for (src_x = srcrect_x; src_x < srcrect_x + srcrect_w; src_x++)
		{
			...
			if (! SDL_ReadSurfacePixel (surf, src_x, src_y, &r, &g, &b, &a))
			{
				printf ("do_copy surface: ERROR: can't get pixel color!\n");
				SDL_UnlockSurface (src);
				return (1);
			}

			SDL_UnlockSurface (src);
			//SDL_GetRGB (pixel, src->format, NULL, &r, &g, &b);

			pixelRGBA (dest_renderer, dst_x, dst_y, r, g, b, a);
...
int do_area_copy (Sint16 x, Sint16 y, Sint16 width, Sint16 height)
{
	...
	copy_area_surface = SDL_CreateSurface (width, height, SDL_PIXELFORMAT_RGBA32);
		...
		switch (event.type)
		{
			case SDL_EVENT_KEY_DOWN:
				...
				case SDL_EVENT_TEXT_INPUT:
					...
					printf ("DEBUG: SDL_EVENT_TEXT_INPUT\n");
...
U1 *get_mouse_state (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	if (! (buttonmask & SDL_BUTTON_MASK (SDL_BUTTON_MIDDLE)))
	{
		mouse_button_middle = 1;
	}

	if (! (buttonmask & SDL_BUTTON_MASK (SDL_BUTTON_RIGHT)))
	{
...
U1 *get_joystick_x2_axis (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	SDL_PumpEvents ();
	x = SDL_GetJoystickAxis (joystick, 2);
...
U1 *get_joystick_y2_axis (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	SDL_PumpEvents ();
	y = SDL_GetJoystickAxis (joystick, 3);
...
U1 *get_joystick_button (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	if (button > SDL_GetNumJoystickButtons (joystick) - 1)
	{
	...
	if (SDL_PollEvent (&event) == 1)
	{
		...
		switch (event.type)
		{
			case SDL_EVENT_JOYSTICK_BUTTON_DOWN:  /* Handle Joystick Button Presses */
	...
	SDL_PumpEvents ();
	button_state = SDL_GetJoystickButton (joystick, button);
	*/
...
U1 *get_joystick_info (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	{
		name_real_len = strlen_safe (SDL_GetJoystickNameForID (0), STRINGMOD_MAXSTRLEN);
