...
U1 *sdl_open_screen (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	*/
	...
	// open SDL window
	window = SDL_CreateWindow ("L1VM", width, height, SDL_WINDOW_OPENGL);
	if (window == NULL)
	{
...
U1 *sdl_open_screen_full (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	*/
	...
	// open SDL window
	window = SDL_CreateWindow ("L1VM", width, height, SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN);
	if (window == NULL)
	{
		printf( "sdl_open_screen: ERROR window can't be opened: %s\n", SDL_GetError ());
...
U1 *sdl_quit (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	if (copy_surface) SDL_DestroySurface (copy_surface);
	if (temp_surface) SDL_DestroySurface (temp_surface);
...
U1 *sdl_text_ttf (U1 *sp, U1 *sp_top, U1 *sp_bottom, U1 *data)
{
	...
	textsurf = TTF_RenderText_Blended (font, (const char *) &data[nameaddr], 0, color);
	if (textsurf == NULL)
	{
